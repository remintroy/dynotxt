...
import { useGetAllUsersQuery, usePutUserDisabledStatusMutation } from "../../../lib/api/authApi";
...
const ManageUsersAllUsersComponent = () => {
  ...
  const { data: allUsersData, isLoading } = useGetAllUsersQuery(page, { skip: !user });
  const [updateDisabledState] = usePutUserDisabledStatusMutation();

  useEffect(() => {
    if (allUsersData) setUserDataList(allUsersData);
  }, [allUsersData]);
  ...
  return (
    <div className="ManageUsersAllUsersComponent">
      ...
      <Box mt={3}>
        {userDataList?.docs?.map((user: any) => (
