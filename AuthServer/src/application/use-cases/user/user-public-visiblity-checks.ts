...
const caseUserPublicVisibiltyCheck = async (
  ...
  if (userData.disabled) throw utilsService.createError(404, "User not found");
  if (userData.privateAccount) throw utilsService.createError(404, "User not found");

  return userData;
};

type caseUserPublicVisibiltyCheck = typeof caseUserPublicVisibiltyCheck;
export default caseUserPublicVisibiltyCheck;
