...
import { useGetFullUserDataQuery, usePutUserDataMutation, usePutUserPersionalDataMutation } from "../../../lib/api/authApi";
import { Avatar, Box, Button, Container, Divider, Flex, Grid, Input, Loader, Select, Text, Transition } from "@mantine/core";
...
const AccountProfilePage = () => {
  ...
  const handleImageUpload = async () => {
    try {
      if (imgFile) {
        setImgLoading(true);
        const storageRef = ref(storageConfig, `users/${user?.uid}/pp.${imgFile.name.split(".")[imgFile.name.split(".").length - 1]}`);
  ...
  return (
    <Transition mounted={mounted} transition="fade" duration={300} timingFunction="ease">
      {(styles) => (
        <Container fluid p={5} style={styles}>
          <NavigationProgress />
          ...
          {user && !isLoading && !isFetching && (
            <Box w={"100%"}>
              ...
              <Box>
                ...
                <br />
                <Button leftIcon={imgLoading ? <Loader size={"sm"} /> : ""} onClick={() => handleImageUpload()} variant="default">
              ...
              <Flex direction={"column"} gap={15} w={"100%"} my={20}>
                ...
                <div>
                  <Button leftIcon={publicLoading ? <Loader size={"sm"} /> : ""} onClick={() => !publicLoading && handlePublicDetalsSubmit()} variant="default">
                    Save Public details
                  </Button>
                </div>
              </Flex>
