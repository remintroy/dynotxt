...
export default function SignUnComponent() {
  ...
  const loginUser = async (type: string) => {
    ...
    try {
      ...
      const confirmPasswordErrorObj = { code: "auth/Confirm-Password-Dosen't-match" };

      if (type === "email" && !email) throw emailRequiredErrorObj;
      if (type === "email" && !password) throw passwordRequiredErrorObj;
      if (type === "email" && password !== confirm) throw confirmPasswordErrorObj;
