...
import { useDeleteFollowUserMutation, useGetFollowUserQuery, usePostFollowUserMutation } from "../../lib/api/authApi";
      ...
      {user?.uid !== userId && (
        <>
          {data?.status && data?.status == "NOTFOLLOWING" && (
            <Button variant="outline" onClick={() => followUserHandler()} leftIcon={followLoading || isFetching || isLoading ? <Loader size="xs" /> : ""}>
              Follow
            </Button>
          )}
          {data?.status && data?.status == "FOLLOWED" && (
            <Button variant="outline" onClick={() => unFollowHandler()} leftIcon={followLoading || isFetching || isLoading ? <Loader size="xs" /> : ""}>
              Unfollow
            </Button>
          )}
